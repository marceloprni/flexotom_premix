...
function createTable() {
    ...
    axios.get("/cadastroLoteMateria/dadosLote").then(response => {
        ...
        tableCadastroLote = jQuery('#tabela-CadastroLote_materia').DataTable(
            {
                pageLength : 4,
                bLengthChange: false,
                bFilter: true,
        ...
        jQuery('#tabela-CadastroLote_materia tbody').on('click', 'tr', function () 
        {
...
btnCriarLote.onclick  = function (event) {
    ...
    axios.post("/cadastroLoteMateria/criarLote", {
...
btnDeleta.onclick = (event) => {
    ...
    axios.delete(`/cadastroLoteMateria/${deleteLote}`).then(response => {
